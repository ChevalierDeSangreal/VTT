...
""" 
    Modified based on track_agileVer2.py
...
class TrackAgileVer3(BaseTask):
    ...
    def pre_physics_step(self):
        ...
        self.root_states[:, 0:3] = 0
        self.root_states[:, 2] = 7
        # reset linevels
        self.root_states[:, 7:10] = 0
        # reset angvels
        self.root_states[:, 10:13] = 0
        # reset quats
        self.root_states[:, 3:7] = 0
        self.root_states[:, 6] = 1

        # $$$ This part if for still target
        self.tar_root_states[:, 0] = 3
        self.tar_root_states[:, 1] = 0
        self.tar_root_states[:, :2] += rand_circle_point(self.num_envs, 1.5, self.device)
        # self.tar_root_states[env_ids, 0:3] = 0
