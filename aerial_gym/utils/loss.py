...
def agile_lossVer5(loss:AgileLoss, quad_state, tar_state, tar_h, tar_ori, tar_dis, step, dt, init_vec, pred_dis):
    ...
    Added aux loss comparing the predicted relative distance with real one
    """
...
def agile_lossVer4(loss:AgileLoss, quad_state, tar_state, tar_h, tar_ori, tar_dis, step, dt, init_vec):
    """
    Based on agile_lossVer3
...
def agile_lossVer3(loss:AgileLoss, quad_state, tar_state, tar_h, tar_ori, tar_dis, step, dt, init_vec):
    ...
    loss_final = 1 * new_loss.ori + 5 * new_loss.distance + 1 * new_loss.vel + 1000 * new_loss.direction + 1 * new_loss.h
    # loss_final = new_loss.distance + new_loss.h
...
def agile_lossVer2(loss:AgileLoss, quad_state, tar_state, tar_h, tar_ori, tar_dis, step, dt, init_vec):
    ...
    # loss_speed = torch.abs(tmp_norm_hor_dis - norm_hor_vel)
    # loss_velocity = torch.norm(rel_vel, dim=1, p=2)
    loss_velocity = torch.norm(vel, dim=1, p=2)
    ...
    loss_final = new_loss.distance * 10 + new_loss.h * 20 + new_loss.ori + new_loss.direction * 1000 + new_loss.vel# + 10 * new_loss.ori
