...
from aerial_gym.utils import task_registry, velh_lossVer5, agile_lossVer1, AgileLoss
...
def get_args():
    custom_parameters = [
        ...
        # model setting
        {"name": "--param_save_path", "type":str, "default": '/home/wangzimo/VTT/VTT/aerial_gym/param_saved/track_agileVer1.pth',
    ...
    with torch.no_grad():
        for epoch in range(args.num_epoch):
            ...
            for step in range(args.len_sample):
                ...
                input_buffer = torch.cat((input_buffer, tmp_input), dim=0).detach()
                ...
                # print("Label:0.25")
                new_state_sim, tar_state = envs.step(new_state_dyn)
                # print("Label:0.5")
                ...
                ave_loss = torch.sum(torch.mul(not_reset_buf, loss))
                sum_loss += ave_loss
                ...
                ave_loss_direciton = torch.sum(loss_direction) / args.batch_size
                ave_loss_distance = torch.sum(loss_distance) / args.batch_size
                ave_loss_velocity = torch.sum(loss_velocity) / args.batch_size
                ...
                writer.add_scalar(f'Direction Loss', loss_direction[0], step)
                writer.add_scalar(f'Distance Loss', loss_distance[0], step)
                writer.add_scalar('Loss Velocity', loss_velocity[0], step)
                writer.add_scalar(f'Orientation Loss', loss_ori[0], step)
                writer.add_scalar(f'Height Loss', loss_h[0], step)
