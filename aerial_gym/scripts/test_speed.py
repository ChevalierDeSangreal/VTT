...
def get_args():
    custom_parameters = [
        ...
        {"name": "--experiment_name", "type": str, "default": "test_speed", "help": "Name of the experiment to run or load."},
        ...
        {"name": "--num_epoch", "type":int, "default": 1, "help": "num of epoch"},
        {"name": "--num_worker", "type":int, "default": 4, "help": "num worker of dataloader"},
        {"name": "--len_sample", "type":int, "default": 200, "help": "length of a sample"},
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for epoch in range(args.num_epoch):
            ...
            for step in range(args.len_sample):
                ...
                for i in range(args.batch_size):
                    speed = torch.norm(now_quad_state[i, 6:8], dim=0, p=2)
                    res_x.append(now_quad_state[i, 0].cpu())
                    res_y.append(now_quad_state[i, 1].cpu())
                    res_z.append(now_quad_state[i, 2].cpu())
                    res_speed.append(speed.cpu())
                    # if step < 10:
    ...
    norm = Normalize(vmin=min(res_speed), vmax=max(res_speed))
    # 使用coolwarm颜色映射
    cmap = cm.coolwarm

    # 将速度转换为颜色值
    colors = cmap(norm(res_speed))
    # 使用plot_trisurf绘制热力图
    surf = ax.plot_trisurf(res_x, res_y, res_z, alpha=1, antialiased=False)
    ...
    plt.savefig(f'/home/wangzimo/VTT/VTT/aerial_gym/scripts/charts_output/speed_hot_plot.png')
    plt.show()
