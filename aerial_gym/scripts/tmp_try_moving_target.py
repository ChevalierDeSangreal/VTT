...
"""
Test program for train_trackagileVer2.py
"""
...
def get_args():
    custom_parameters = [
        ...
        {"name": "--experiment_name", "type": str, "default": "test__track_agileVer1", "help": "Name of the experiment to run or load."},
    ...
    with torch.no_grad():
        for epoch in range(args.num_epoch):
            ...
            for step in range(args.len_sample):
                ...
                # print("Label:0.25")
                new_state_sim, tar_state = envs.step(now_quad_state.detach())
                # print("Label:0.5")
                ...
                writer.add_scalar(f'Position/X', tar_pos[0, 0], step)
                writer.add_scalar(f'Position/Y', tar_pos[0, 1], step)
                writer.add_scalar(f'Velocity/X', tar_state[0, 6], step)
                writer.add_scalar(f'Velocity/Y', tar_state[0, 7], step)
                ...
                # writer.add_scalar(f'Speed/Z', tar_state[0, 8], step)
                # writer.add_scalar(f'Speed', speed, step)
                writer.add_scalar(f'Height', tar_state[0, 2], step)
