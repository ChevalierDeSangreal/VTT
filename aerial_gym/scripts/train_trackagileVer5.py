...
def get_args():
    custom_parameters = [
        ...
        # train setting
        {"name": "--learning_rate", "type":float, "default": 5.6e-8,
...
if __name__ == "__main__":
    ...
    for epoch in range(args.num_epoch):
        ...
        if (epoch + 1) % 50 == 0:
            print("Saving Model...")
            model_weights = {
                'extractor': model_extractor.state_dict(),
                'decision': model_decision.state_dict()
            }
            # torch.save(model.state_dict(), args.param_save_path)
            torch.save(model_weights, args.param_save_path)
